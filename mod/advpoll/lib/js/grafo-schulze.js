//necesita cargar kinetic con elgg_load_js('kinetic');
...
function grafo_condorcet (id, votaciones) {
    window.onload = function() {
		...
		var dame_posicion_nodos = function (num_nodos) {
			...
			radionodo = parseInt(ancho/(num_nodos*2.5));
			...
			resultados = dame_vertices_poligono (inicial_x, inicial_y, num_nodos, parseInt(ancho/(num_nodos/2)));
		...
		var dibuja_nodo = function (centro, etiqueta) {
			...
			nodo.on('mouseover', function(){
				...
				for(var n = 0; n < puntos.length; n++) {
					...
					puntuacion.setFontSize(parseInt(radionodo/2));
			...
			nodo.on("mouseout", function(){
				...
				for(var n = 0; n < puntos.length; n++) {
					...
					puntuacion.setFontSize(parseInt(radionodo/3));
					layer.draw();
				};
		...
		var dibuja_flecha = function (origen, destino, etiqueta, nodo) {
			...
			var texto = new Kinetic.Text({
				x: parseInt((destino_x+origen_x)/2), 
				y: parseInt((destino_y+origen_y)/2),
				text: etiqueta,
				fontSize: parseInt(radionodo/3),
				fontFamily: "Arial",
				fill: "white",
				strokeWidth: 1,
				stroke: "grey",
